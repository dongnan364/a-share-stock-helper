...
def derive_themes(record: Dict[str, Any]) -> List[str]:
    text = " ".join(
        str(part or "")
        for part in [record.get("sector"), record.get("company"), record.get("name"), record.get("concepts")]
    ).lower()
    themes: List[str] = []

    ai_keywords = [
        "人工智能", "aigc", "ai", "大模型", "生成式", "机器学习", "深度学习", "算法",
        "知识图谱", "自然语言", "多模态", "智能语音", "智能驾驶", "无人驾驶", "自动驾驶",
        "算力", "算力租赁", "智算", "东数西算", "数据中心", "idc", "服务器", "液冷",
        "温控", "cpo", "光模块", "光通信", "光芯片", "gpu", "cpu", "fpga", "asic",
        "存储芯片", "dram", "nand", "hbm", "半导体设备", "先进封装", "chiplet",
        "pcb", "高速铜缆", "边缘计算", "云计算",
    ]
    ai_power_words = ["电力", "能源", "储能", "变压器", "ups", "配电", "电源", "温控", "液冷"]
    ai_power_context = ["算力", "智算", "数据中心", "idc", "服务器", "液冷", "温控", "东数西算"]

    robot_keywords = [
        "机器人", "人形机器人", "具身智能", "世界模型", "机械臂", "协作机器人", "服务机器人",
        "工业机器人", "机器人本体", "减速器", "谐波减速器", "rv减速器", "伺服", "伺服电机",
        "伺服系统", "控制器", "执行器", "关节模组", "灵巧手", "丝杠", "滚珠丝杠",
        "直线导轨", "传感器", "力矩传感器", "六维力", "机器视觉", "视觉检测", "激光雷达",
        "slam", "运动控制", "工业自动化", "智能工厂", "自动化设备", "工控", "plc", "数控系统",
    ]

    if any(keyword.lower() in text for keyword in ai_keywords) or (
        any(keyword.lower() in text for keyword in ai_power_context)
        and any(keyword.lower() in text for keyword in ai_power_words)
    ):
        themes.append("AI")
    if any(keyword.lower() in text for keyword in robot_keywords):
        themes.append("机器人")
    return themes
